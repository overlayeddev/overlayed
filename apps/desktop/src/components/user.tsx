...
export const User = ({ item, alignDirection }: { item: OverlayedUser, alignDirection: DirectionLR }) => {
  ...
  return (
    <div className={`flex gap-2 py-1 p-2 justify-start items-center ${alignDirection == "right" ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`pointer-events-none relative rounded-full border-2 ${avatarClass} ${talkingClass}`}>
        ...
        <div className={`absolute left-[10px] bottom-[-4px] bg-black rounded-full text-red-500 ${alignDirection == "center" ? "flaex" : "md:hidden"}`}>
